...
public class RedisJava {
    ...
    public void testString() {
        //添加数据
        jedis.set("name", "zhouhuan"); //key为name放入value值为chx
        ...
        //s设置多个键值对
        jedis.mset("name", "zhouhuan", "age", "24", "email", "17726055317@163.com");
    ...
    public void testMap() {
        ...
        map.put("sex","male");
        map.put("name", "zhouhuan");
        map.put("age", "100");
        map.put("email", "17726055317@163.com");
        jedis.hmset("user", map);

        //取出user中的name，结果是一个泛型的List
        //第一个参数是存入redis中map对象的key，后面跟的是放入map中的对象的key，后面的key是可变参数
        List<String> list = jedis.hmget("user", "name", "age", "email","sex");
        System.out.println(list);

        //删除map中的某个键值
        jedis.hdel("user", "age");                   //删除操作age
    ...
    public void testSet(){
        ...
        jedis.sadd("user","啦啦啦");
        jedis.sadd("user","蔡徐坤");
        jedis.sadd("user","鸡你太美");
        jedis.sadd("user","CXK");
        jedis.sadd("user","鸡你真的太美");
        //移除user集合中的元素are
        jedis.srem("user","are");
        jedis.srem("user","chen");
        jedis.srem("user","xiyu");
        jedis.srem("user","chx","CXK");
    ...
    public void test(){
        ...
        jedis.lpush("number","9");//将一个或多个值插入到列表头部（最左边）
        jedis.lpush("number","1");
        jedis.lpush("number","19");
